...
void vRemoveAllDevices(PrimaryDevice_t *pstcDeviceHead) {
  Device_t *pstcTempDevice = pstcDeviceHead->pstcNextDevice;
...
uint8_t ui8PutDevicenameIntoBuffer(PrimaryDevice_t *pstcDeviceHead, uint8_t* pui8Buf, uint8_t ui8DeviceIndex) {
  uint8_t ui8Size = 0;
  Device_t *pstcTempDevice = pstcGetDeviceByIndex(pstcDeviceHead, ui8DeviceIndex);
  if (pstcTempDevice != NULL) {
    ui8Size = pstcTempDevice->ui8ServiceRecordLength;
    memcpy(pui8Buf, pstcTempDevice->pui8ServiceRecord, ui8Size);
  }
  return ui8Size;
}
