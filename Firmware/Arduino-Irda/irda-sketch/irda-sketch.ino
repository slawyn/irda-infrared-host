...
PowerBuffer_t stcTransmissionQueue;
...
uint8_t arrui8IrCommPayload[dBufferSize-4-2-2] = {0};

#define log(size,data) {vLog(&stcTransmissionQueue,size,data);};
...
uint8_t ui8IrDaCommService(Connection_t *pstcConnection, uint8_t *pui8Status) {
  ...
  *pui8Status = 0;
  log(2, &pstcConnection->ui8IrCommState);
        ...
        if (stcLapData.ui16Indication) {
          ...
          if (ui8GetDevicesCount(&stcIrDa.stcPrimaryDevice) > 0 && !i32FbitTimer) {
            if ((pstcSecondaryDevice = pstcGetDeviceByIndex(&stcIrDa.stcPrimaryDevice, 0)) != NULL) {
              ...
              vDeleteAllDevices(&stcIrDa.stcPrimaryDevice);
            }
        ...
        if (ui8Request) {
          ...
          if (!stcIrDa.ui8RemoteBusy) {
            if (stcUnackedFrameStorage.ui8UnackedFrameCount > 0) {
              log(1, &stcUnackedFrameStorage.ui8UnackedFrameCount);
...
#define dPassDebugErrorFrame() {stcIrDaReceiveBuffer.ui8Length = stcIrDaReceiveBuffer.ui8BytesWritten;vLogIrDa(&stcTransmissionQueue,&stcIrDaReceiveBuffer,&stcIrDa);stcIrDaReceiveBuffer.ui8Length = 0;}
      ...
      switch (ui8SirRxState) {
        ...
        case dRxDecoderData:
          if (ui8Data == dSTF) {
            ...
            vLogIrDa(&stcTransmissionQueue, &stcIrDaReceiveBuffer, &stcIrDa);                // For debugging, paass the frame with all the errors
...
inline void vControl(uint32_t ui8TimeDelta) {
  HeapBuffer_t *pstcCurrentHeapBuffer;
  static uint16_t ui16Command = STATE_WAIT;
  static uint16_t ui16Timeout = 0;
  static uint8_t ui8FrameLength = 0;
  static uint8_t *pui8ReceivedData = NULL;
  static uint8_t ui8FrameRead  = 0;
  ...
  if ((ui16Command != STATE_WAIT) && ((ui16Timeout += ui8TimeDelta) >= dPcCommTimeout)) {
  ...
  switch (ui16Command) {
    case CMD_DEBUG:
      pstcCurrentHeapBuffer = pstcAllocateNextHeapBuffer(&stcTransmissionQueue, 5);
      pstcCurrentHeapBuffer->arrui8Buffer[0] = 5;
      pstcCurrentHeapBuffer->arrui8Buffer[0] = (CMD_DEBUG);
      pstcCurrentHeapBuffer->arrui8Buffer[0] = (CMD_DEBUG >> 8);
      pstcCurrentHeapBuffer->arrui8Buffer[0] = (0);
      pstcCurrentHeapBuffer->arrui8Buffer[0] = (stcIrDa.ui8LapState);
    ...
    case CMD_CALCULATE_CRC:
      ui16Temp = ui16CalculateFcs(&pui8ReceivedData[2], ui8FrameLength - 3);
      pstcCurrentHeapBuffer = pstcAllocateNextHeapBuffer(&stcTransmissionQueue, 6);
      pstcCurrentHeapBuffer->arrui8Buffer[0] =  6;
      pstcCurrentHeapBuffer->arrui8Buffer[1] =  CMD_CALCULATE_CRC;
      pstcCurrentHeapBuffer->arrui8Buffer[2] = (CMD_CALCULATE_CRC >> 8);
      pstcCurrentHeapBuffer->arrui8Buffer[3] =  0;
      pstcCurrentHeapBuffer->arrui8Buffer[4] = ui16Temp;
      pstcCurrentHeapBuffer->arrui8Buffer[5] = ui16Temp >> 8;
      ui16Command = STATE_CLEANUP;
      break;
    case CMD_GET_STATISTIC:
      pstcCurrentHeapBuffer = pstcAllocateNextHeapBuffer(&stcTransmissionQueue, 27);
      pstcCurrentHeapBuffer->arrui8Buffer[0] =  27;
      pstcCurrentHeapBuffer->arrui8Buffer[1] =  CMD_GET_STATISTIC;
      pstcCurrentHeapBuffer->arrui8Buffer[2] = (CMD_GET_STATISTIC >> 8);
      pstcCurrentHeapBuffer->arrui8Buffer[3] =  0;
      memcpy(&pstcCurrentHeapBuffer->arrui8Buffer[4],&stcIrDa.stcStatistics, sizeof(stcIrDa.stcStatistics));
      ui16Command = STATE_CLEANUP;
      break;
    case CMD_GET_DEVICES:
      ui16Temp = ui8GetTotalDevicenamesLength(&stcIrDa.stcPrimaryDevice);
      pstcCurrentHeapBuffer = pstcAllocateNextHeapBuffer(&stcTransmissionQueue, ui16Temp + 5);
      pstcCurrentHeapBuffer->arrui8Buffer[0] =  ui16Temp + 5;
      pstcCurrentHeapBuffer->arrui8Buffer[1] =  CMD_GET_DEVICES;
      pstcCurrentHeapBuffer->arrui8Buffer[2] = (CMD_GET_DEVICES >> 8);
      pstcCurrentHeapBuffer->arrui8Buffer[3] =  0;
      pstcCurrentHeapBuffer->arrui8Buffer[4] =  ui8GetDevicesCount(&stcIrDa.stcPrimaryDevice);
      vPutDevicenamesIntoBuffer(&stcIrDa.stcPrimaryDevice,&pstcCurrentHeapBuffer->arrui8Buffer[5]);
      ui16Command = STATE_CLEANUP;
      break;
    case CMD_RESET_STATS:
      pstcCurrentHeapBuffer = pstcAllocateNextHeapBuffer(&stcTransmissionQueue, 4);
      pstcCurrentHeapBuffer->arrui8Buffer[0] =  4;
      pstcCurrentHeapBuffer->arrui8Buffer[1] =  CMD_RESET_STATS;
      pstcCurrentHeapBuffer->arrui8Buffer[2] = (CMD_RESET_STATS >> 8);
      pstcCurrentHeapBuffer->arrui8Buffer[3] =  0;

      memset(&stcIrDa.stcStatistics, 0, sizeof(stcIrDa.stcStatistics));
    ...
    case CMD_IRCOMM_PAYLOAD:
      memcpy(arrui8IrCommPayload, &pui8ReceivedData[2], ui8FrameLength - 3);
      pstcCurrentHeapBuffer = pstcAllocateNextHeapBuffer(&stcTransmissionQueue, 4);
      pstcCurrentHeapBuffer->arrui8Buffer[0] =  4;
      pstcCurrentHeapBuffer->arrui8Buffer[1] =  CMD_IRCOMM_PAYLOAD;
      pstcCurrentHeapBuffer->arrui8Buffer[2] = (CMD_IRCOMM_PAYLOAD >> 8);
      pstcCurrentHeapBuffer->arrui8Buffer[3] =  arrui8IrCommPayload[0];
      ui16Command = STATE_CLEANUP;
      break;
    case STATE_WAIT:
      ui8Available = dPcComDataAvailable();
      if (ui8Available > 0) {
        ui8FrameLength = dPcComRead();
        if (ui8FrameLength > dHeapBufferMaxMemory || ui8FrameLength < 3) {
          ui16Command  = STATE_CLEANUP;
        } else {
          ui16Timeout = 0;
          pui8ReceivedData = malloc(ui8FrameLength - 1);
    ...
    case STATE_COMMAND:
      ui8Available = dPcComDataAvailable();
      while (ui8Available > 0) {
        --ui8Available;
        pui8ReceivedData[ui8FrameRead++] = dPcComRead();
        if ((ui8FrameLength-1) ==  ui8FrameRead) {
          ui16Command = *((uint16_t*)&pui8ReceivedData[0]);
    ...
    case STATE_TIMEOUT:
      pstcCurrentHeapBuffer = pstcAllocateNextHeapBuffer(&stcTransmissionQueue, 4);
      pstcCurrentHeapBuffer->arrui8Buffer[0] =  4;
      pstcCurrentHeapBuffer->arrui8Buffer[1] =  CMD_STATUS;
      pstcCurrentHeapBuffer->arrui8Buffer[2] = (CMD_STATUS >> 8);
      pstcCurrentHeapBuffer->arrui8Buffer[3] =  0xFF;
    case STATE_CLEANUP:
    default:
      if (pui8ReceivedData != NULL) {
        free(pui8ReceivedData);
        pui8ReceivedData = NULL;
      }
      ui8FrameRead = 0;
  ...
  // Send frames from the queue to pc
  if ((pstcCurrentHeapBuffer = pstcHandleHextHeapBuffer(&stcTransmissionQueue)) != NULL) {
    ui8Available = dPcComRoom();


    // Write whole
    if ((pstcCurrentHeapBuffer->ui8Length - stcTransmissionQueue.ui8WrittenLength) <= ui8Available) {
      dPcComWriteBytes(pstcCurrentHeapBuffer->arrui8Buffer + stcTransmissionQueue.ui8WrittenLength, pstcCurrentHeapBuffer->ui8Length - stcTransmissionQueue.ui8WrittenLength);
      stcTransmissionQueue.ui8WrittenLength = pstcCurrentHeapBuffer->ui8Length;
      // Split writes
    } else {
      dPcComWriteBytes(pstcCurrentHeapBuffer->arrui8Buffer + stcTransmissionQueue.ui8WrittenLength, pstcCurrentHeapBuffer->ui8Length - stcTransmissionQueue.ui8WrittenLength - ui8Available);
      stcTransmissionQueue.ui8WrittenLength = pstcCurrentHeapBuffer->ui8Length - ui8Available;
    }
  }
}
...
void setup() {
  ...
  // Reset structures
  memset((uint8_t*)&stcTransmissionQueue, 0, sizeof(stcTransmissionQueue));
